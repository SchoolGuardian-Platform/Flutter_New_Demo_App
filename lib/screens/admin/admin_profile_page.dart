...
/// new endpoint -- an admin account is a `User` like any other, so no
/// admin-specific backend work is needed to show it.
///
/// There is no `PATCH /auth/me` (profile-edit) endpoint on the backend
/// yet, so this is read-only for now; the edit button is disabled with an
/// explanatory tooltip rather than silently doing nothing.
///
/// "Change Password" is different: the backend has no logged-in
/// change-password endpoint (old password + new password), only the
/// existing `POST /auth/forgot-password` email flow used by
/// `AuthService.forgotPassword`. So this button reuses that -- it sends a
/// reset link to the admin's own address rather than changing anything
/// in place. See `_changePassword` below.
...
class _AdminProfilePageState extends State<AdminProfilePage> {
  ...
  /// Sends a password-reset email to the admin's own address, reusing
  /// `POST /auth/forgot-password` -- there is no separate "change
